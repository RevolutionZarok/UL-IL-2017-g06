%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
/*  DISCONTIGUOUS PREDICATES  */
%:- multifile systemTestCaseStep/7.
:- multifile msrTest/1.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%-----------------------------------------------
:-msrTestAddStep([system,sim,1,7]).
%-----------------------------------------------
msrTest([[system, sim, 1,7],
[[target, oeAlert],
 [context, Context],
 [inputParameters, InputParameters],
 [outputParameters, OutputParameters],
 [comments, Comments],
 TestResult]
]):-
%-----------------------------------------------
(
 (
% Step 7

%% Variables Declaration
msrVar(ctState, TheSystem),
msrVar(actComCompany, TheActor),
msrVar(etHumanKind, AetHumanKind),
msrVar(dtDate, AdtDate),
msrVar(dtTime, AdtTime),
msrVar(dtPhoneNumber, AdtPhoneNumber),
msrVar(dtGPSLocation, AdtGPSLocation),
msrVar(dtComment, AdtComment),
msrVar(dtSMS, AdtSMS),

%% Constraints

theSystem(TheSystem),

msrNav([AetHumanKind],[],[[etHumanKind, witness]]),
msrNav([AdtDate],[year, value],[[ptInteger,2017]]),
msrNav([AdtDate],[month, value],[[ptInteger,11]]),
msrNav([AdtDate],[day, value],[[ptInteger,26]]),
msrNav([AdtTime],[hour, value],[[ptInteger,10]]),
msrNav([AdtTime],[minute, value],[[ptInteger,10]]),
msrNav([AdtTime],[second, value],[[ptInteger,16]]),
msrNav([AdtPhoneNumber],[value],[[ptString,'+3524666445252']]),
msrNav([AdtGPSLocation],[latitude, value],[[ptReal,49.627675]]),
msrNav([AdtGPSLocation],[longitude, value],[[ptReal,6.159590]]),
msrNav([AdtComment],[value],[[ptString,'3 cars involved in an accident.']]),

%% Test Specification
Target =  msrNav,
	ParametersList = 
	[ [TheActor],
	  [rnInterfaceOUT, oeAlert, [AetHumanKind,AdtDate,AdtTime,AdtPhoneNumber,AdtGPSLocation,AdtComment]],
	  [Result]
	],
	!,
GoalGet=..[Target | ParametersList],

%% Oracle specification
OracleGet=..[true]
)
->
%% Test Interpretation
((GoalGet,!)
-> ((OracleGet,!)
	-> TestResult = [success]
	; TestResult = [failedAtOracle])
; TestResult = [failedAtGoal]
)
; TestResult = [failedAtTestDeclarationOrSpecification]
),
%% Test Outcome
Context = [['TheSystem', TheSystem]],
Parameters = [['TheActor' ,TheActor],['AetHumanKind' ,AetHumanKind],['AdtDate' ,AdtDate],['AdtTime' ,AdtTime],['AdtPhoneNumber' ,AdtPhoneNumber],['AdtGPSLocation' ,AdtGPSLocation],['AdtComment' ,AdtComment],['AdtSMS' ,AdtSMS]],
OutputParameters = [],
Comments = ''
.
